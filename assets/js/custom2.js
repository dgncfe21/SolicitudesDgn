...
$(document).ready(function(){
    ...
    // var table = $('#tablaINST_MEDS_MTM').DataTable({
    ...
    // .appendTo('#tablaINST_MEDS_MTM_wrapper .col-md-6:eq(0)');
...
const config = {
    ...
    measurementId: "G-SZLNLRFQVT"
      ...
      var coleccionINST_MEDS_MTM = db.ref().child("INST_MEDS_MTM");
      ...
      coleccionINST_MEDS_MTM.on("child_added", datos => {        
          dataSet = [datos.key, datos.child("medidor").val().substring(1,7), datos.child("instalo").val().slice(1,-1), datos.child("orden").val().slice(1,-1),datos.child("comentarios").val().slice(1,-1),datos.child("fecha_hora").val().slice(1,-1),datos.child("latitud").val(),datos.child("longitud").val()];
          table.rows.add([dataSet]).draw();
          
      });


      coleccionINST_MEDS_MTM.on('child_changed', datos => {           
          dataSet = [datos.key, datos.child("medidor").val(), datos.child("instalo").val(), datos.child("orden").val(),datos.child("comentarios").val(),datos.child("fecha_hora").val(),datos.child("latitud").val(),datos.child("longitud").val()];
          table.row(filaEditada).data(dataSet).draw();
      });
      coleccionINST_MEDS_MTM.on("child_removed", function() {
      ...
      $('form').submit(function(e){                         
        ...
        //   let longitud = $.trim($('#longitud').val());                               
          let idFirebase = id;        
          if (idFirebase == ''){                      
              idFirebase = coleccionINST_MEDS_MTM.push().key;
          };                
          data = {medidor:medidor, instalo:instalo, orden:orden, comentarios:comentarios, fecha_hora:fecha_hora, latitud:latitud, longitud:longitud};             
          actualizacionData = {};
          actualizacionData[`/${idFirebase}`] = data;
          coleccionINST_MEDS_MTM.update(actualizacionData);
          id = '';        
          $("form").trigger("reset");
...
/*       $('#btnNuevo').click(function() {
    ...
    ///
      ...
      $("#tablaINST_MEDS_MTM").on("click", ".btnEditar", function() {    
          filaEditada = table.row($(this).parents('tr'));           
          let fila = $('#tablaINST_MEDS_MTM').dataTable().fnGetData($(this).closest('tr'));               
          let id = fila[0];
          console.log(id);
      ...
      $("#tablaINST_MEDS_MTM").on("click", ".btnBorrar", function() {   
          filaEliminada = $(this);
          Swal.fire({
          ...
          if (result.value) {
              let fila = $('#tablaINST_MEDS_MTM').dataTable().fnGetData($(this).closest('tr'));            
              let id = fila[0];            
              db.ref(`INST_MEDS_MTM/${id}`).remove()
              Swal.fire('¡Eliminado!', 'El servicio ha sido eliminado.','success')
          }
          })        
      });  */

      $('#tablaINST_MEDS_MTM').DataTable({
        dom: 'Bfrtip',
        buttons: [
            {
                extend: 'print',
    ...
    function printData() {
        var divToPrint = document.getElementById("tablaINST_MEDS_MTM");
        newWin = window.open("");
      ...
      function printCertificate() {
        const printContents = document.getElementById('tablaINST_MEDS_MTM').outerHTML;
